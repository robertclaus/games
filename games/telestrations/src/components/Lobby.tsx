...
export function Lobby({ onJoin }: LobbyProps) {
  ...
  return (
    <div style={s.page}>
      <div style={s.card}>
        <div style={s.title}>✏️ Telestrations</div>
        <p style={s.subtitle}>The telephone game meets Pictionary · 4–8 players</p>
...
export function WaitingRoom({ roomId, players, isHost, myPlayerId, onStart, onGoHome }: WaitingRoomProps) {
  ...
  const canStart = n >= 4 && n <= 8;
  const startLabel = !canStart
    ? n < 4 ? `Need ${4 - n} more player${4 - n !== 1 ? 's' : ''}` : 'Max 8 players'
    : `Start Game (${n} players, ${n} rounds)`;
    ...
    <div style={s.page}>
      <div style={s.card}>
        ...
        <p style={{ color: '#64748B', fontSize: '0.8rem', textAlign: 'center' }}>
          4–8 players · each player gets one booklet · number of rounds = number of players
        </p>
...
const s: Record<string, React.CSSProperties> = {
  ...
  btnPrimary:    { padding: '12px', borderRadius: 6, border: 'none', background: '#8B5CF6', color: '#fff', fontSize: '1rem', fontWeight: 700, cursor: 'pointer' },
  ...
  roomCodeValue: { fontFamily: 'monospace', fontSize: '1.4rem', fontWeight: 800, letterSpacing: '4px', color: '#E2E8F0', background: 'rgba(139,92,246,0.15)', padding: '4px 12px', borderRadius: 6, border: '1px solid #8B5CF6' },
  ...
  badge:         { background: '#4C1D95', color: '#C4B5FD', padding: '2px 8px', borderRadius: 4, fontSize: '0.75rem' },
  badgeHost:     { background: '#8B5CF6', color: '#fff', padding: '2px 8px', borderRadius: 4, fontSize: '0.75rem', fontWeight: 700 },
};
